...
public class ControlledHorseEntity extends Horse {
    ...
    public void setStanding(boolean shouldStand) {
        ...
        if (shouldStand) {
            List<LivingEntity> kickedEntites = level().getEntitiesOfClass(LivingEntity.class, new AABB(this.position(), this.position()).inflate(2), e -> !this.getPassengers().contains(e));
            for (LivingEntity livingEntity : kickedEntites) {
                if (livingEntity.equals(this)) continue;
    ...
    public void setHeadRotation(float headRotation) {
        this.entityData.set(HEAD_ROTATION, headRotation);
    }
    public float getBodyRotation() {
        return this.entityData.get(BODY_ROTATION);
    }
    public void setBodyRotation(float bodyRotation) {
        this.entityData.set(BODY_ROTATION, bodyRotation);
    }
    public float getCruiseSpeed() {
        return this.entityData.get(SPEED);
    }
    public void setCruiseSpeed(float cruiseSpeed) {
        this.entityData.set(SPEED, cruiseSpeed);
    }
    public int getMaxSpurs() {
        return this.entityData.get(MAX_SPURS);
    }
    public void setMaxSpurs(int value) {
        this.entityData.set(MAX_SPURS, value);
    }
    public float getSpurs() {
        return this.entityData.get(SPURS);
    }
    public void setSpurs(float value) {
        this.entityData.set(SPURS, value);
    }
    ...
    public void readAdditionalSaveData(@NotNull CompoundTag tag) {
        ...
        setMaxSpurs(tag.getInt("MaxSpurs"));
    ...
    protected float getRiddenSpeed(@NotNull Player player) {
        ...
        return (float) switch(speed) {
            case 1 -> 0.2 * this.getAttributeValue(Attributes.MOVEMENT_SPEED);
            case 2 -> 0.5 * this.getAttributeValue(Attributes.MOVEMENT_SPEED);
            case 4 -> 1.5 * this.getAttributeValue(Attributes.MOVEMENT_SPEED);
            default -> this.getAttributeValue(Attributes.MOVEMENT_SPEED);
        };
    }
